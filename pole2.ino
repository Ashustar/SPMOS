...
char auth[] = "PNbIafr9cCd4CcOyjaSKlynCSzh3oWQw";
...
void blynkAnotherDevice() // Here we will send HIGH or LOW once per second
{
  ...
  if (digitalRead(powerLDR)) {
    ...
    bridge1.virtualWrite(V22, 0); // Sends 1 value to BLYNK_WRITE(V5) handler on receiving side.
  Serial.println("OK");
  digitalWrite(D4, LOW);
  
  } else {
    digitalWrite(D0, LOW); // Digital Pin 9 on the second board will be set LOW
    bridge1.virtualWrite(V22, 1); // Sends 0 value to BLYNK_WRITE(V5) handler on receiving side.
     Serial.println("PROBLEM AT POLE 2 ");
